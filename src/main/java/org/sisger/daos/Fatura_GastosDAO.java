...
public class Fatura_GastosDAO {
	...
	public List<Fatura_Gastos> listarFaturaCreditoAberto(Metodo metodo, Situacao sit){
		...
		return manager.createQuery("from Fatura_Gastos fg where fg.metodotipo.tipo=:tipo and fg.situacao.nome=:nome",Fatura_Gastos.class).setParameter("tipo",
				metodo.getTipo()).setParameter("nome",sit.getNome()).getResultList();
	}
